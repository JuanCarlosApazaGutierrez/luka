...
def obtener_detalle_caso(request):
    ...
    if codigo_caso:
        try:
            caso = Casos.objects.get(id_caso=codigo_caso, activate=True)
            data = {
...
def editar_huella(request):
    ...
    return render(request, "huellas/editar_huella.html", {'casos_activos': casos_activos, 'error_message': 'Error al procesar la imagen'})
...
def agregar_caso(request):
    ...
    if request.method == "POST":
        if request.POST.get("nombre_caso") and request.POST.get("descripcion") and request.POST.get("codigo_caso") and request.POST.get("departamento"):
            ...
            caso.tipo = request.POST.get("tipo")
            caso.save()
...
def cambiar_estado_huella(request, id_caso):
    ...
    # caso.save()
    # return JsonResponse({'success': True, 'activate': caso.activate})
    return render(request, "casos/listar_caso.html")
...
def agregar_implicado(request):
    if request.method == 'POST':        
        nuevo_implicado = Implicado(
            ...
            sexo=request.POST.get('implicado_sexo'),
            activo=True  
        )
...
def procesar_imagen(request):
    if request.method == 'POST' and request.FILES.get('imagen'):
        ...
        selected_x = request.POST.get('selected_x')
        selected_y = request.POST.get('selected_y')
        print(selected_x)
        print(selected_y)
        if selected_x and selected_y:
             nucleus_coords = (float(selected_x), float(selected_y)) 
        else:
            nucleus_coords = buscar_nucleo_esqueleto(imag)
...
def process_image(request):
    if request.method == "POST":
        ...
        if 'image' not in request.FILES:
            return JsonResponse({'error': 'No se proporcionó una imagen'})
        ...
        # Instancia del algoritmo y actualización de parámetros
        preproc = PreprocesamientoHuellas()
        try:
            preproc.factor_escala_relativo_x = float(request.POST.get('factor_escala_relativo_x', 0.65))
            preproc.factor_escala_relativo_y = float(request.POST.get('factor_escala_relativo_y', 0.65))
            preproc.incremento_angulo = int(request.POST.get('incremento_angulo', 3))
            preproc.longitud_de_onda_maxima = int(request.POST.get('longitud_de_onda_maxima', 15))
            preproc.longitud_de_onda_minima = int(request.POST.get('longitud_de_onda_minima', 5))
            preproc.sigma_bloque = int(request.POST.get('sigma_bloque', 7))
            preproc.sigma_gradiente = int(request.POST.get('sigma_gradiente', 1))
            preproc.sigma_suavizado_orientacion = int(request.POST.get('sigma_suavizado_orientacion', 7))
            preproc.tam_bloque_frecuencia_crestas = int(request.POST.get('tam_bloque_frecuencia_crestas', 38))
            preproc.tam_bloque_segmento_crestas = int(request.POST.get('tam_bloque_segmento_crestas', 16))
            preproc.tam_ventana_frecuencia_crestas = int(request.POST.get('tam_ventana_frecuencia_crestas', 5))
            preproc.umbral_filtro_crestas = float(request.POST.get('umbral_filtro_crestas', -3))
            preproc.umbral_segmento_crestas = float(request.POST.get('umbral_segmento_crestas', 0.1))
            preproc.nuevas_filas_redimensionar = int(request.POST.get('nuevas_filas_redimensionar', 350))
        except Exception as e:
            return JsonResponse({'error': 'Error al convertir los parámetros: ' + str(e)})
        
        # Ejecuta el algoritmo de preprocesamiento
        try:
            processed_img = preproc.mejorar(img, redimensionar=True)
        except Exception as e:
            return JsonResponse({'error': str(e)})
        
        # Convierte la imagen binaria (bool) a uint8 para poder codificarla a JPEG.
        # Se asume que True será 255 (blanco) y False 0 (negro)
        processed_img_uint8 = (processed_img * 255).astype(np.uint8)
        
        # Codifica la imagen resultante a JPEG y luego a base64 para enviarla
        retval, buffer = cv2.imencode('.jpg', processed_img_uint8)
        jpg_as_text = base64.b64encode(buffer).decode('utf-8')
        return JsonResponse({'processed_image': jpg_as_text})
...
def aplicar_filtros(request):
    if request.method == 'POST':
        ...
        if 'enhance1' in filters:
            enhancer = ImageEnhance.Contrast(img_pil)
            img_pil = enhancer.enhance(float(filters['enhance1']) / 5)
            img_cv = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)

        if 'median1' in filters:
            ksize = int(filters['median1'])
            if ksize % 2 == 0:  # cv2.medianBlur necesita un número impar
                ksize += 0.1
            img_cv = cv2.medianBlur(img_cv, ksize)

        if 'gaussian1' in filters:
            ksize = int(filters['gaussian1'])
            if ksize % 2 == 0:
                ksize += 0.1
            img_cv = cv2.GaussianBlur(img_cv, (ksize, ksize), 0)
        
        if 'threshold1' in filters:
            _, img_cv = cv2.threshold(cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY), int(filters['threshold1']), 255, cv2.THRESH_BINARY)
        
        if 'dilation1' in filters:
            kernel = np.ones((int(filters['dilation1']), int(filters['dilation1'])), np.uint8)
            img_cv = cv2.dilate(img_cv, kernel, iterations=1)
        
        if 'erosion1' in filters:
            kernel = np.ones((int(filters['erosion1']), int(filters['erosion1'])), np.uint8)
            img_cv = cv2.erode(img_cv, kernel, iterations=1)
        
        # Convertir imagen nuevamente a base64
        _, buffer = cv2.imencode('.png', img_cv)
        img_base64 = base64.b64encode(buffer).decode('utf-8')
        processed_image = f"data:image/png;base64,{img_base64}"

        return JsonResponse({'processed_image': processed_image})
...
def encode_image(img):
    if img.dtype == bool:  # Si la imagen está en formato booleano
...
@csrf_exempt
def binalizado_automatico(request):
    if request.method == 'POST':
        try:
            # Procesar la primera imagen enviada (campo 'imagen')
            imagen1_file = request.FILES.get('imagen')
            if not imagen1_file:
                return JsonResponse({"error": "No se recibió la imagen 1"}, status=400)
            imagen_array1 = np.frombuffer(imagen1_file.read(), np.uint8)
            imagen_cv2_1 = cv2.imdecode(imagen_array1, cv2.IMREAD_GRAYSCALE)
            if imagen_cv2_1 is None:
                return JsonResponse({"error": "Error al leer la imagen 1"}, status=400)
            
            # Procesar la segunda imagen enviada (campo 'imagen2')
            imagen2_file = request.FILES.get('imagen2')
            if not imagen2_file:
                return JsonResponse({"error": "No se recibió la imagen 2"}, status=400)
            imagen_array2 = np.frombuffer(imagen2_file.read(), np.uint8)
            imagen_cv2_2 = cv2.imdecode(imagen_array2, cv2.IMREAD_GRAYSCALE)
            if imagen_cv2_2 is None:
                return JsonResponse({"error": "Error al leer la imagen 2"}, status=400)
            
            # Instanciar y procesar cada imagen con el método 'mejorar'
            preprocesador1 = PreprocesamientoHuellas()
            resultado_img1 = preprocesador1.mejorar(imagen_cv2_1)
            
            preprocesador2 = PreprocesamientoHuellas()
            resultado_img2 = preprocesador2.mejorar(imagen_cv2_2)
            
            # Codificar los resultados en base64 para enviarlos al frontend
            result1 = encode_image(resultado_img1)
            result2 = encode_image(resultado_img2)
            
            return JsonResponse({
                "imagen_procesada1": result1,
                "imagen_procesada2": result2
            })
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
...
def convertir_serializable(obj):
    """
    Función recursiva para convertir objetos NumPy a tipos nativos de Python.
    """
    if isinstance(obj, np.generic):
        ...
        return [convertir_serializable(elem) for elem in obj]
    else:
        return obj
